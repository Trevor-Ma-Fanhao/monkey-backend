...
import org.monkey.project.common.ResultUtils;
import org.monkey.project.exception.BusinessException;
import org.monkey.project.mapper.UserInterfaceInfoMapper;
import org.monkey.project.model.vo.InterfaceInfoVO;
...
public class AnalysisController {
    ...
    /*
    * 获取调用次数最多的接口信息列表
    * 通过用户接口信息表 查询调用次数最多的接口ID 再关联查询接口详细信息
    ...
    * */
    ...
    public BaseResponse<List<InterfaceInfoVO>> listBaseResponse(){
        ...
        // 设置查询条件 使用接口信息ID在接口信息映射中的键集合 进行条件匹配
        queryWrapper.in("id", interfaceInfoIdObjMap.keySet());
        // 调用接口信息服务的list方法 传入条件包装器 获取符合条件的接口信息列表
        // 接口信息服务的list方法 会去查看interface_info 中的接口信息 is_delete 是否为1 若1则不会列出
